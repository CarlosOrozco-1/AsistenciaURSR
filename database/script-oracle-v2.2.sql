...
-- -- Versión: 2.2 (Corrección de Trigger de Auditoría)
...
-- --------------------------------------------------------------------------------
-- SECCIÓN 5: CREACIÓN DE VISTAS (VERSIÓN OPTIMIZADA)
-- --------------------------------------------------------------------------------
PROMPT Creando vistas para simplificar consultas...
CREATE OR REPLACE VIEW V_REPORTE_ASISTENCIA_DIARIA AS
SELECT
    TRUNC(RA.FECHA_HORA_REGISTRO) AS FECHA_ASISTENCIA,
    I.ID_INSCRIPCION,
    E.NUMERO_CARNET,
    E.NOMBRES || ' ' || E.APELLIDOS AS NOMBRE_ESTUDIANTE,
    C.NOMBRE_CURSO,
    S.CODIGO_SECCION,
    -- Técnica de agregación condicional: más eficiente que subconsultas.
    MIN(CASE WHEN RA.TIPO_REGISTRO = 'ENTRADA' THEN RA.FECHA_HORA_REGISTRO END) AS HORA_ENTRADA,
    MAX(CASE WHEN RA.TIPO_REGISTRO = 'SALIDA' THEN RA.FECHA_HORA_REGISTRO END) AS HORA_SALIDA,
...
-- --------------------------------------------------------------------------------
-- SECCIÓN 8: TRIGGERS DE AUDITORÍA SIMPLIFICADOS (VERSIÓN CORREGIDA)
-- --------------------------------------------------------------------------------
...
-- SP_AUDITAR_TABLA con la firma correcta de parámetros.
...
BEGIN
    ...
    ELSIF UPDATING THEN
        ...
        v_valores_anteriores := 'CARNET: ' || :OLD.NUMERO_CARNET || ', NOMBRES: ' || :OLD.NOMBRES;
        v_valores_nuevos := 'CARNET: ' || :NEW.NUMERO_CARNET || ', NOMBRES: ' || :NEW.NOMBRES;
    ELSIF DELETING THEN
    ...
    SP_AUDITAR_TABLA(
        ...
        p_usuario_app => NULL -- Se puede enriquecer desde el backend si se configura.
    );
END;
/

-- TRIGGER CORREGIDO Y MEJORADO
...
BEGIN
    IF INSERTING THEN
        v_tipo_accion := 'INSERT';
        v_valores_nuevos := 'ID_INSCRIPCION: ' || :NEW.ID_INSCRIPCION_FK || ', TIPO: ' || :NEW.TIPO_REGISTRO;
    ELSIF UPDATING THEN
        -- Aunque nuestro flujo no actualiza registros de asistencia, es una buena práctica tenerlo.
        v_tipo_accion := 'UPDATE';
        v_valores_anteriores := 'ID_INSCRIPCION: ' || :OLD.ID_INSCRIPCION_FK || ', TIPO: ' || :OLD.TIPO_REGISTRO;
        v_valores_nuevos := 'ID_INSCRIPCION: ' || :NEW.ID_INSCRIPCION_FK || ', TIPO: ' || :NEW.TIPO_REGISTRO;
    ELSIF DELETING THEN
        v_tipo_accion := 'DELETE';
        v_valores_anteriores := 'ID_INSCRIPCION: ' || :OLD.ID_INSCRIPCION_FK || ', TIPO: ' || :OLD.TIPO_REGISTRO;
    ...
    -- Llamada corregida con todos los parámetros requeridos
    SP_AUDITAR_TABLA(
        p_nombre_tabla       => 'REGISTROS_ASISTENCIA',
        p_tipo_accion        => v_tipo_accion,
        p_id_registro        => NVL(TO_CHAR(:NEW.ID_REGISTRO), TO_CHAR(:OLD.ID_REGISTRO)),
        p_valores_anteriores => v_valores_anteriores, -- El valor anterior es nulo en una inserción.
        p_valores_nuevos     => v_valores_nuevos,     -- El valor nuevo es nulo en una eliminación.
        p_usuario_app        => NULL -- Se puede enriquecer desde el backend si se configura.
    );
END;
/
